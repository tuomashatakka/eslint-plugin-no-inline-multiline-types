...
const rule = {
  meta: {
    type: "suggestion",
    docs: {
      description: "Disallows inline TSTypeLiteral annotations that span multiple lines",
      recommended: false,
      url: null,
    },
    ...
    schema: [],
    messages: {
      multilineInlineType: 'Inline type literal annotations should not span multiple lines. Extract to a named interface or type alias.',
    },
  },
  ...
  create(context) {
    ...
    function checkMultilineLiteralInAnnotation(typeAnnotationNode) {
      ...
      if (literalNode && literalNode.type === 'TSTypeLiteral') {
        if (literalNode.loc.start.line !== literalNode.loc.end.line) {
          context.report({
            node: literalNode, // Report the TSTypeLiteral itself
            messageId: "multilineInlineType",
            fix: function (fixer) {
              // --- Fix Logic ---
